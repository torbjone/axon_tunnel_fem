...
for e_idx in range(num_elecs):
    ...
    lead_field[e_idx, :] = np.load(join(results_folder, "phi_sources_elec_{}.npy".format(e_idx)))#[closest_idxs]

phi = np.dot(lead_field, imem)
print(phi.shape, tvec.shape)
# print(phi)

phi_norm = np.max(np.abs(phi)) #/ 50
for elec in range(num_elecs):
    plt.plot(tvec, phi[elec] / phi_norm)
